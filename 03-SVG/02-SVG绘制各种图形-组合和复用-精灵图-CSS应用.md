...
在 SVG 画布中，插入一个形状，需要在文档中创建对应的元素。使用不同的属性，来定义图形的大小和位置。
...
用 SVG 绘制一条折线（多段线），使用 `<polyline>` 元素
...
- 与折线很像，都是由连接一组点的直线构成。
- 不同的是，`<polygon>` 的路径在最后一个点处**自动闭合**（矩形也是一种多边形，也可以用 `<polygon>` 创建）。
- 默认会填充黑色。
...
- SVG 中最常见的绘制方式；
- `<path>` 元素可以绘制矩形（直角矩形或圆角矩形）、圆形、椭圆、折线、多边形，以及一些其他的形状；
  - 例如贝塞尔曲线、2 次曲线等曲线。
- 默认会填充黑色。
- 默认路径不会闭合。
...
- ~~`xlink:href`~~：（SVG 2.0 已弃用）需要复制的元素/片段的 URL 或 ID 。
- `x`、`y` ：元素的坐标（相对复制元素的位置），默认值：0。
- `width`、`height`：元素的宽和高，
  - 仅在引入有 `viewBox` 属性的元素时，即 `<svg>` 或 `<symbol>` 元素才起作用；
  - 默认值：0。
...
- `<symbol>` 元素可以使用属性，如：`viewBox`、`preserveAspectRatio` 、`x`、`y`、`width`、`height` 等。`<defs>` 元素没有专有属性。
...
- 方案一：使用元素的属性，比如：`fill` 属性、`stroke ` 属性等。
- 方案二：使用 CSS 样式，svg 也是 HTML 中的元素，也支持用 CSS 编写样式。
...
CSS 样式可写在 `<defs>` 中，也可写在 HTML 中作为内联样式或外部样式引入。
...
- 行内 `style` > `<defs>` 中的内联 `<style>` > 外部 / 内联 `<style>` > 元素上的属性，如 `fill`
