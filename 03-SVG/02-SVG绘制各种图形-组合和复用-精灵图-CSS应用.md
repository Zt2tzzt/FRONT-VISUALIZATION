...
- 多边形和折线很像，它们都是由连接一组点的直线构成。不同的是，<polygon> 的路径在最后一个点处自动闭合（矩形也是一种多边形，也可以用多边形创建）。
- 默认会填充黑色。
...
- 大写字母，表示采用绝对 定位。另一种是用小写字母，表示采用相对定位。
...
CSS样式可写在 <defs> 中，也可写在 HTML 头部或外部等）。
...
- 内联的 style > defs 中的 style > 外部 / head 内部 > 属性，如 fill
